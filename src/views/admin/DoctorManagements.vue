<template>
  <div class="w-full flex justify-center">
    <!-- 创建病例对话框 -->
    <el-dialog v-model="showCreate" top="5vh" title="Create Doctor Record" width="60%">
      <el-form ref="createFormRef" :model="newRecord" label-position="top" label-width="auto">
        <div class="flex flex-row w-full space-x-[2rem]">
          <el-form-item label="ID" prop="id" class="w-1/2" required>
            ...
            <span class="text-gray-400">The ID of the doctor record</span>
          </el-form-item>
        ...
        <el-form-item label="Is Revoked" prop="isRevoked" required>
    ...
    <!-- 更新病例对话框 -->
    <el-dialog v-model="showUpdate" top="5vh" title="Update Doctor Record" width="60%">
    ...
    <div class="flex flex-col w-full h-full px-[4rem] py-[2rem] space-y-[1rem]">
      ...
      <div class="flex overflow-hidden rounded-md shadow-md border">
        <div class="flex w-full h-[5rem] items-center">
          ...
          <span class="m-[0.75rem] text-2xl font-600">Doctor Records</span>
        </div>
      </div>
      ...
      <el-card class="h-max">
        ...
        <el-table :data="filteredRecords" height="360" class="w-full max-h-full">
          ...
          <el-table-column prop="Operation" label="Operation" min-width="200px">
            <template #default="scope">
              <el-button size="small" type="warning" circle @click="revokeRecord(scope.row)" :icon="PauseOne" />
...
import { MedicalFiles, Search, Delete, RefreshOne, PauseOne } from '@icon-park/vue-next';
import request from '@/axios'; // 确保你已经配置好 axios 实例

const records = ref([
  { id: 1, time: '2024-06-01 12:00', isRevoked: false },
  { id: 2, time: '2024-06-02 14:30', isRevoked: false }
]);
