    ...
    <div class="flex h-full justify-center items-center">
      <div class="h-max min-w-[16rem] w-1/4 max-w-[24rem] text-center items-center">
        <div class="inline-flex mt-4 mb-8 items-center">
          ...
          <h1 class="font-bold text-4xl font-mono">NEU</h1>
        </div>
...
const login = async (form) => {
  ...
  let success = function(role) {
    ...
    console.log('Logged in user:', getUser()); // 调试信息，登录后获取的用户信息
    router.push('/');
  }
  ...
  await form.validate((valid, fields) => {
    if (valid) {
      request.post("/api/v1/auth/token", {
        ...
        ElMessage({
          message: "Input invalid: " + error.message,
          type: "error",
        });
      });
    } else {
