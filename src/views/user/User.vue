  ...
  <div class="w-full flex justify-center">
    <div class="flex flex-col w-full h-full px-[4rem] py-[2rem] space-y-[1rem]">
      ...
      <el-card class="h-max">
        ...
        <el-table :data="filteredUsers" class="w-full max-h-full">
...
const filteredUsers = computed(() => {
  if (isAdmin()) {
    ...
    return filter.value.filter(user => user.id === currentUser.value.id);
  }
});
...
const updateUser = () => {
  ...
  form.validate((valid) => {
    if (valid) {
      request.put("/api/v1/users/" + updatedUser.value.id, updatedUser.value).then(() => {
